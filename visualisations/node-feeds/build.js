...
$__System.register("1", ["3", "9", "b"], function (_export) {
  "use strict";

  var d3, _, $, percent, percent4dp;
    ...
    execute: function () {
      ...
      $("#select_file").on("change", function () {
        ...
        d3.json("output/" + filename, function (error, data) {
          ...
          var root = d3.select("#view").html("").classed("open", true);

          root.datum(data.result).call(buildTree);
          ...
          function buildTree(selection) {
            selection.each(function (data) {
              ...
              if (data.values) {
                var container = root.append("ul").attr("class", "values");

                container.selectAll("li").data(_.sortBy(data.values, "value")).enter().append("li").append("span").attr("class", "content").text(function (d) {
                ...
                if (data.too_many_values) {
                  container.append("li").append("span").attr("class", "content too_many_values").text("too many values");
                }
              }

              if (data.list) {
                root.append("ul").attr("class", "list").append("li").datum(data.list).call(buildTree);
              }
