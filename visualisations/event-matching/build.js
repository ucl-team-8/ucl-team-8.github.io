  ...
  function draw() {
    ...
    loadData().then(function (_ref3) {
      ...
      var services = d3.nest().key(function (d) {
        ...
        return d3.ascending(a.seq, b.seq);
      }).entries(trustData);
      ...
      var unitServices = undefined;

      try {
        unitServices = _.find(services, function (d) {
          return d.key == unit.key;
        }).values;
      } catch (e) {
        console.error(e);
        return;
      ...
      var svg = d3.select("body").append("svg").attr("width", width).attr("height", height).attr("id", "visualisation_svg");
      ...
      var mergedServices = _.flatten(unitServices.map(function (d) {
        return d.values;
      }));

      var serviceDiagram = svg.append("g").attr("class", "service-diagram").attr("transform", "translate(150,0)");

      var serviceStops = serviceDiagram.selectAll(".stop").data(mergedServices).enter().append("g").attr("class", "stop");
